...
glm::mat4 Matrix_Look(glm::vec4 position_c, glm::vec4 view_vector, glm::vec4 up_vector, float speed, float delta_t, int opcao)
{
    ...
    if (opcao == 0)
        position_c += -w * speed * delta_t;
    else if (opcao == 1)
        position_c += w * speed * delta_t;
    else if (opcao == 2)
        position_c += u * speed * delta_t;
    else if (opcao == 3)
        position_c += -u * speed * delta_t;
...
glm::vec4 atualiza_c(glm::vec4 position_c, glm::vec4 view_vector, glm::vec4 up_vector, float speed, float delta_t, int opcao)
{
    glm::vec4 w = -view_vector / norm(view_vector)/* PREENCHA AQUI o cálculo do vetor w */;
    glm::vec4 u = crossproduct(up_vector, w) / norm(crossproduct(up_vector, w))/* PREENCHA AQUI o cálculo do vetor u */;

    if (opcao == 0)
        return position_c += -w * speed * delta_t;
    else if (opcao == 1)
        return position_c += w * speed * delta_t;
    else if (opcao == 2)
        return position_c += u * speed * delta_t;
    else if (opcao == 3)
        return position_c += -u * speed * delta_t;
    return position_c;
